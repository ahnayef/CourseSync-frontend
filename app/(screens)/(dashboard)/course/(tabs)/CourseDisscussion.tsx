...
const courseDiscussion = ({ course }: any) => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const res = await request.get(`/discussion/questions/${course.id}`);
    ...
    if (course.id) {
      ...
      newSocket.emit("joinRoom", { course_id: course.id });
